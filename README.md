...
> ⚠️ **Production readiness — multi-process LSPs**: the wire-ceremony poison TX defense is now implemented for **3 of 4** ceremony paths (PRs #136 sub-factory advance, #137 leaf advance, #138 leaf realloc).  Multi-process LSPs running these flows produce a fully-signed L-stock / sales-stock poison TX via a second MuSig2 round bundled with each state advance — the watchtower receives a real signed poison TX and can redistribute the stake to clients on cheating.  The remaining Tier B root-rollover ceremony (`lsp_run_state_advance`, fires ~once per N leaf advances) still uses the single-process fallback in multi-process mode and degrades to NULL poison_tx with a SECURITY GAP stderr warning — operators see this in logs.  See [`docs/poison-tx.md`](docs/poison-tx.md) for the full security model.  Single-process / signet self-tests / `--demo` deployments have full poison TX protection across all paths.
