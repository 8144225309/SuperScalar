...
> v0.1.3 — 520 tests, 7-job CI, encrypted transport (Noise NK), SQLite persistence. **Ready for signet/testnet collaborative testing.**
...
| **Testing** | 418 unit + 43 regtest + 30 orchestrator + 29 manual flag tests, CI on every push (Linux, macOS, sanitizers, cppcheck, coverage, fuzz) |
...
461 automated tests (418 unit + 43 regtest integration, including 11 adversarial/edge-case tests) plus 29 manual flag tests and 30 orchestrator scenarios. CI runs automated suites on every push — Linux, macOS, AddressSanitizer, cppcheck static analysis, coverage, and libFuzzer.
