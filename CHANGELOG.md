...
- **Bridge payment state persistence**: Bridge HTLC paths (`MSG_BRIDGE_ADD_HTLC`, `MSG_BRIDGE_PAY_RESULT`) now persist channel balance, commitment number, PCS, PCP, and HTLC state after each commitment exchange. Previously, bridge-originated payments updated in-memory state but never called `persist_update_channel_balance()`, so killing the LSP after bridge payments caused irreconcilable commitment number mismatches on reconnect. The fulfill path also now waits for `REVOKE_AND_ACK` and processes revocation (watchtower, bidirectional revocation) instead of fire-and-forget.
