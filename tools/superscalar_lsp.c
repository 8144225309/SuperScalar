...
int broadcast_factory_tree(factory_t *f, regtest_t *rt,
...
/* Broadcast all signed factory tree nodes, waiting for real block confirmations.
   ...
   Returns 1 on success. */
int broadcast_factory_tree_any_network(factory_t *f, regtest_t *rt,
                                                const char *mine_addr,
                                                int is_regtest,
                                                int confirm_timeout) {
    for (size_t i = 0; i < f->n_nodes; i++) {
...
int main(int argc, char *argv[]) {
    ...
    if (n_payments > 0 || daemon_mode || demo_mode || breach_test || test_expiry ||
        ...
        /* === Populate shared test context for extracted test functions === */
        tctx.mgr = mgr;
        tctx.lsp = &lsp;
        tctx.ctx = ctx;
        tctx.lsp_kp = &lsp_kp;
        tctx.lsp_seckey = lsp_seckey;
        tctx.rt = &rt;
        tctx.rpt = &rpt;
        tctx.db = use_db ? &db : NULL;
        tctx.g_db = g_db;
        tctx.lad = lad;
        tctx.fee_est = fee_est;
        tctx.mine_addr = mine_addr;
        tctx.fund_spk = fund_spk;
        tctx.fund_addr = fund_addr;
        tctx.n_clients = n_clients;
        tctx.n_total = n_total;
        tctx.step_blocks = step_blocks;
        tctx.states_per_layer = states_per_layer;
        tctx.leaf_arity = leaf_arity;
        tctx.level_arities = level_arities;
        tctx.n_level_arity = n_level_arity;
        tctx.active_blocks = active_blocks;
        tctx.dying_blocks = dying_blocks;
        tctx.cltv_timeout_arg = cltv_timeout_arg;
        tctx.funding_sats = funding_sats;
        tctx.is_regtest = is_regtest;
        tctx.use_db = use_db;
        tctx.confirm_timeout_secs = confirm_timeout_secs;
        tctx.breach_test = breach_test;
        tctx.init_local = init_local;
        tctx.init_remote = init_remote;
        tctx.network = network;
        tctx.g_shutdown = &g_shutdown;

        /* === DW Exhibition Test (extracted) === */
        if (test_dw_exhibition && channels_active) {
            int rc = lsp_test_dw_exhibition(&tctx);
            if (rc >= 0) {
                if (use_db) persist_close(&db);
                lsp_cleanup(&lsp);
                memset(lsp_seckey, 0, 32);
        ...
        /* === Leaf Advance Test (extracted) === */
        if (test_leaf_advance) {
            int rc = lsp_test_leaf_advance(&tctx);
            if (rc >= 0) {
                if (use_db) persist_close(&db);
                lsp_cleanup(&lsp);
                memset(lsp_seckey, 0, 32);
        ...
        /* === Dual Factory Test (extracted) === */
        if (test_dual_factory && channels_active) {
            int rc = lsp_test_dual_factory(&tctx);
            if (rc >= 0) {
                if (use_db) persist_close(&db);
                lsp_cleanup(&lsp);
                memset(lsp_seckey, 0, 32);
        ...
        /* === Bridge Test (extracted) === */
        if (test_bridge && mgr->n_channels > 0) {
            lsp_test_bridge(&tctx);
            /* Bridge test does not exit early -- continues to cooperative close */
        }
    ...
    /* === Breach Test (extracted) === */
    if (breach_test && channels_active) {
        int rc = lsp_test_breach(&tctx);
        if (rc >= 0) {
            if (use_db) persist_close(&db);
            lsp_cleanup(&lsp);
            memset(lsp_seckey, 0, 32);
    ...
    /* === Expiry Test (extracted) === */
    if (test_expiry && channels_active) {
        int rc = lsp_test_expiry(&tctx);
        if (rc >= 0) {
            if (use_db) persist_close(&db);
            lsp_cleanup(&lsp);
            memset(lsp_seckey, 0, 32);
    ...
    /* === Distribution TX Test (extracted) === */
    if (test_distrib && channels_active) {
        int rc = lsp_test_distrib(&tctx);
        if (rc >= 0) {
            if (use_db) persist_close(&db);
            lsp_cleanup(&lsp);
            memset(lsp_seckey, 0, 32);
    ...
    /* === PTLC Key Turnover Test (extracted) === */
    if (test_turnover && channels_active) {
        int rc = lsp_test_turnover(&tctx);
        if (rc >= 0) {
            if (use_db) persist_close(&db);
            lsp_cleanup(&lsp);
            memset(lsp_seckey, 0, 32);
    ...
    /* === Auto-Rebalance Test (extracted) === */
    if (test_rebalance && channels_active) {
        int rc = lsp_test_rebalance(&tctx);
        if (rc == 1) {
            jit_channels_cleanup(mgr);
            free(mgr);
            if (use_db) persist_close(&db);
    ...
    /* === Batch-Rebalance Test (extracted) === */
    if (test_batch_rebalance && channels_active) {
        int rc = lsp_test_batch_rebalance(&tctx);
        if (rc == 1) {
            jit_channels_cleanup(mgr);
            free(mgr);
            if (use_db) persist_close(&db);
    ...
    /* === Leaf Realloc Test (extracted) === */
    if (test_realloc && channels_active) {
        int rc = lsp_test_realloc(&tctx);
        if (rc == 1) {
            jit_channels_cleanup(mgr);
            free(mgr);
            if (use_db) persist_close(&db);
