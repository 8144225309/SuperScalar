...
static void usage(const char *prog) {
    fprintf(stderr,
        ...
        "  --cheat-backup-restore[=OFFSET]  #256. On the BOLT type-136 channel_reestablish path (peer_mgr reconnect / ln_dispatch), inject a stale per_commitment_point for commitment_number-OFFSET (default 1). Exercises type-136 DLP. NOTE: SuperScalar clients reconnect over JSON MSG_RECONNECT -- the client-side stale-state defense is the RECONNECT_ACK untrusted-claim path (test_regtest_reconnect_cheat.sh, #94). LSP only.\n"
