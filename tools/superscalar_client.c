...
static int daemon_channel_cb(int fd, channel_t *ch, uint32_t my_index,
    ...
    /* Epoch reset state (persists between Round 1 and Round 2) */
    secp256k1_musig_secnonce epoch_secnonces[FACTORY_MAX_NODES];
    ...
    while (!g_shutdown) {
        ...
        case MSG_EPOCH_RESET_PROPOSE: {
            /* LSP proposes epoch reset — distributed 2-round MuSig2 signing.
               Round 1: receive LSP nonces, reset epoch, generate client nonces, reply.
               Round 2: receive ALL nonces, finalize, create psigs, reply. */
            int er_round;
            wire_bundle_entry_t er_entries[FACTORY_MAX_NODES * FACTORY_MAX_SIGNERS];
            size_t er_n_entries = 0;
            if (!wire_parse_epoch_reset_propose(msg.json, &er_round,
                                                  er_entries,
                                                  FACTORY_MAX_NODES * FACTORY_MAX_SIGNERS,
                                                  &er_n_entries)) {
                fprintf(stderr, "Client %u: bad EPOCH_RESET_PROPOSE\n", my_index);
                cJSON_Delete(msg.json);
                break;
            }
            cJSON_Delete(msg.json);
            printf("Client %u: EPOCH_RESET_PROPOSE round %d (%zu entries)\n",
                   my_index, er_round, er_n_entries);

            if (er_round == 1) {
                /* Reset epoch + rebuild unsigned txs locally */
                if (!factory_reset_epoch_unsigned(factory)) {
                    fprintf(stderr, "Client %u: factory_reset_epoch_unsigned failed\n",
                            my_index);
                    break;
                }
                if (!factory_sessions_init(factory)) {
                    fprintf(stderr, "Client %u: factory_sessions_init failed\n",
                            my_index);
                    break;
                }

                /* Set received nonces (LSP's nonces) */
                for (size_t ei = 0; ei < er_n_entries; ei++) {
                    secp256k1_musig_pubnonce pn;
                    if (!musig_pubnonce_parse(ctx, &pn, er_entries[ei].data))
                        break;
                    factory_session_set_nonce(factory, er_entries[ei].node_idx,
                                               er_entries[ei].signer_slot, &pn);
                }

                /* Generate client's nonces for all nodes this client signs */
                unsigned char er_seckey[32];
                if (!secp256k1_keypair_sec(ctx, er_seckey, keypair))
                    break;

                wire_bundle_entry_t my_nonce_entries[FACTORY_MAX_NODES];
                size_t n_my_nonces = 0;
                memset(epoch_secnonces, 0, sizeof(epoch_secnonces));
                ...
                for (size_t n = 0; n < factory->n_nodes; n++) {
                    ...
                    if (!musig_generate_nonce(ctx, &epoch_secnonces[n], &my_pn,
                                               er_seckey, &my_pubkey,
                                               &factory->nodes[n].keyagg.cache)) {
                        memset(er_seckey, 0, 32);
                        fprintf(stderr, "Client %u: nonce gen failed node %zu\n",
                                my_index, n);
                        break;
                    }
                    factory_session_set_nonce(factory, n, (size_t)slot, &my_pn);

                    my_nonce_entries[n_my_nonces].node_idx = (uint32_t)n;
                    my_nonce_entries[n_my_nonces].signer_slot = (uint32_t)slot;
                    musig_pubnonce_serialize(ctx, my_nonce_entries[n_my_nonces].data,
                                              &my_pn);
                    my_nonce_entries[n_my_nonces].data_len = 66;
                    n_my_nonces++;
                }
                memset(er_seckey, 0, 32);

                epoch_reset_round1_done = 1;

                /* Reply with EPOCH_RESET_PSIG: client nonces, empty psigs */
                cJSON *r1_reply = wire_build_epoch_reset_psig(
                    my_nonce_entries, n_my_nonces, NULL, 0);
                wire_send(fd, MSG_EPOCH_RESET_PSIG, r1_reply);
                cJSON_Delete(r1_reply);

                printf("Client %u: sent Round 1 nonces (%zu entries)\n",
                       my_index, n_my_nonces);

            } else if (er_round == 2) {
                if (!epoch_reset_round1_done) {
                    fprintf(stderr, "Client %u: Round 2 without Round 1\n", my_index);
                    break;
                }
                ...
                /* Finalize + create partial sigs for nodes this client signs */
                unsigned char er_seckey2[32];
                if (!secp256k1_keypair_sec(ctx, er_seckey2, keypair))
                    break;

                secp256k1_keypair er_kp;
                if (!secp256k1_keypair_create(ctx, &er_kp, er_seckey2)) {
                    memset(er_seckey2, 0, 32);
                    break;
                }
                memset(er_seckey2, 0, 32);
                ...
                for (size_t n = 0; n < factory->n_nodes; n++) {
                    ...
                    if (!factory_session_finalize_node(factory, n)) {
                        fprintf(stderr, "Client %u: finalize failed node %zu\n",
                                my_index, n);
                        break;
                    }

                    /* Create partial sig using secnonce from Round 1 */
                    secp256k1_musig_partial_sig ps;
                    if (!musig_create_partial_sig(ctx, &ps, &epoch_secnonces[n],
                                                    &er_kp,
                                                    &factory->nodes[n].signing_session)) {
                        fprintf(stderr, "Client %u: psig failed node %zu\n",
                                my_index, n);
                        break;
                    }

                    my_psig_entries[n_my_psigs].node_idx = (uint32_t)n;
                    my_psig_entries[n_my_psigs].signer_slot = (uint32_t)slot;
                    musig_partial_sig_serialize(ctx, my_psig_entries[n_my_psigs].data,
                                                 &ps);
                    my_psig_entries[n_my_psigs].data_len = 32;
                    n_my_psigs++;
                }

                epoch_reset_round1_done = 0;

                /* Reply with EPOCH_RESET_PSIG: empty nonces, client psigs */
                cJSON *r2_reply = wire_build_epoch_reset_psig(
                    NULL, 0, my_psig_entries, n_my_psigs);
                wire_send(fd, MSG_EPOCH_RESET_PSIG, r2_reply);
                cJSON_Delete(r2_reply);

                printf("Client %u: sent Round 2 psigs (%zu entries)\n",
                       my_index, n_my_psigs);

                /* Persist DW counter state */
                if (cbd && cbd->db) {
