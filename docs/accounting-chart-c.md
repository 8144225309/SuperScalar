...
| PS-advance | — | — | ✓ `test_regtest_econ_ps_advance` (in-process); on-chain chain advance + sweep covered by `test_regtest_full_force_close_and_sweep_arity_ps_chain_len2` and `..._chain_len5` (Phase 2 #3); on-chain N=8 / N=16 scale + heterogeneous chain mixes + adversarial non-revocability covered by `test_regtest_ps_full_lifecycle_n{8,16}`, `test_regtest_ps_heterogeneous_chains_n{8,16}`, and `test_regtest_ps_old_state_broadcast_fails_n8` (Phase 3 #1); on-chain N=32 scale (122 nodes, 31 channel sweeps + 31 L-stock sweeps, 32-way MuSig at root) + heterogeneous chain depths covered by `test_regtest_ps_full_lifecycle_n32` and `test_regtest_ps_heterogeneous_chains_n32` (Phase 3 #2) |
