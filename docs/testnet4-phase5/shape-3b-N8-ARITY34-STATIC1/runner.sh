#!/usr/bin/env bash
# Phase 5 runner — Shape 3b (N=8 ARITY=3,4 STATIC_NEAR_ROOT=1)
...
case "$VARIANT" in
    ...
    *)  echo "FAIL: unknown VARIANT=$VARIANT (V1|V2)" >&2; exit 2 ;;
esac
...
emit() { printf '%s\n' "$*" >> "$EVIDENCE"; }
emit "# Shape 3b $VARIANT — incremental evidence"
...
echo "=== Phase 5 Shape 3b $VARIANT (N=8 ARITY=3,4 STATIC_NEAR_ROOT=1) ==="

pkill -9 -f "superscalar_(lsp|client|watchtower).*--port $PORT" 2>/dev/null || true

nohup "$LSP_BIN" \
    --network "$NETWORK" --port "$PORT" \
    --clients 8 --arity 3,4 --static-near-root 1 \
