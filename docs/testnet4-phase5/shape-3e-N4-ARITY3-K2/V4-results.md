# shape-3e-N4-ARITY3-K2 V4 - Results

See `V4-commands.md` for the launch protocol. Filled in as the run progresses.

## Run record
- Wallet:    ss_pool_8
- Started:   `<UTC>`
- Completed: `<UTC>`
- Duration:  `<wall>`
- Funding rate: 1.0 sat/vB
- **Outcome: pending**

## On-chain evidence
| Event | Block | TXID | Notes |
|---|---|---|---|
| Funding TX                     | TBD | TBD | from ss_pool_8 |
| Factory root / leaf broadcasts | TBD | TBD | |
| Sub-factory chain[N] TXs       | TBD | TBD | (k2 shapes only) |
| Force-close commits            | TBD | TBD | (V2/V5/V6 only) |
| Per-client sweeps              | TBD | TBD | |
| Sales-stock sweeps             | TBD | TBD | (k2 shapes only) |
| WT response TX                 | TBD | TBD | (V5 only) |

## Log assertions observed
- [ ] `MSG_SUBFACTORY_PROPOSE`
- [ ] `MSG_SUBFACTORY_DONE`
- [ ] `ps_subfactory_chain entry.*chain_len=2`

## Conservation
- Funding in:   TBD sats
- Sweeps out:   TBD sats
- Fees total:   TBD sats (all at <=1 sat/vB)
- Conservation: `X == Y + Z`  [ ]

## Sweep-back
- ss_pool_8 -> superscalar_test
- TXID:    TBD
- Block:   TBD
- Sats:    TBD in / TBD fee at 1 sat/vB

## Failure / observation notes
(empty)
