# shape-3f-N64-ARITY24-K2-STATIC1 V5 - Commands

Proves: INTEGRATION breach + WT: LSP broadcasts stale sub-factory state; WT detects + broadcasts response TX before CSV closes.
...
## Launch
```
ssh root@68.168.216.243 'systemd-run --unit=ss-t4-phase5-3f-V5.service \
  --setenv WALLET=ss_pool_6 --setenv TAG=phase5_3f_V5 --setenv VARIANT=V5 \
  bash /root/SuperScalar-phase5/docs/testnet4-phase5/shape-3f-N64-ARITY24-K2-STATIC1/runner.sh'
```
Variant V5 -> `--demo --breach-test`. Port 9954.

## Monitor
```
ssh root@68.168.216.243 'tail -f /tmp/ss_t4_phase5_3f_V5_lsp.log'
ssh root@68.168.216.243 'journalctl -u ss-t4-phase5-3f-V5.service -f'
```
...
```
fee_rate=1 sat/vB. Echoes sweep-back txid; capture into V5-results.md.

## Stop (if needed)
```
ssh root@68.168.216.243 'systemctl stop ss-t4-phase5-3f-V5.service'
```
