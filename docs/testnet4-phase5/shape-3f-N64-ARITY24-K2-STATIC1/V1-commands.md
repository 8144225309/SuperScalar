# shape-3f-N64-ARITY24-K2-STATIC1 V1 - Commands

Proves: INTEGRATION: scale (N=64) + mixed-arity (2,4) + static-near-root + k2 sub-factory simultaneously. Lifecycle / coop close.
...
## Launch
```
ssh root@68.168.216.243 'systemd-run --unit=ss-t4-phase5-3f-V1.service \
  --setenv WALLET=ss_pool_2 --setenv TAG=phase5_3f_V1 --setenv VARIANT=V1 \
  bash /root/SuperScalar-phase5/docs/testnet4-phase5/shape-3f-N64-ARITY24-K2-STATIC1/runner.sh'
```
Variant V1 -> `--demo`. Port 9954.

## Monitor
```
ssh root@68.168.216.243 'tail -f /tmp/ss_t4_phase5_3f_V1_lsp.log'
ssh root@68.168.216.243 'journalctl -u ss-t4-phase5-3f-V1.service -f'
```
...
## Stop (if needed)
```
ssh root@68.168.216.243 'systemctl stop ss-t4-phase5-3f-V1.service'
```
